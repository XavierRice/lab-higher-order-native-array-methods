...
function printAllSongTitles(songs) {
  songs.forEach(albums => console.log (albums.title))
}
...
function printSongDetails(songs) {
  songs.forEach(albums => console.log (albums.title + " by " + albums.artist ))
}
...
function printSongTitlesOverThreeMinutes(songs) {
  songs.filter(song => song.runtimeInSeconds > 180).forEach(song => console.log(song.title))
}
